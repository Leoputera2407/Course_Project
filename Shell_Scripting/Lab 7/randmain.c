...
main (int argc, char **argv)
{
  ...
  if (rdrand_supported ())
	...
	dl_handle = dlopen("randlibhw.so", RTLD_NOW);
        if (!dl_handle)
        {
            fprintf(stderr, "dlopen() randlibhw.so failed: %s\n", dlerror());
            return 1;
        }
      rand64 = dlsym(dl_handle, "hardware_rand64");
	if (dlerror())
        {
            fprintf(stderr, "dlsym() hardware_rand64: %s\n", dlerror());
	    dlclose(dl_handle);
            return 1;
        }
    }
	...
	dl_handle = dlopen("randlibsw.so", RTLD_NOW);
        if (!dl_handle)
        {
            fprintf(stderr, "dlopen() randlibsw.so failed: %s\n", dlerror());
            return 1;
        }
      rand64 = dlsym(dl_handle, "software_rand64");
	if (dlerror())
        {
            fprintf(stderr, "dlsym() software_rand64: %s\n", dlerror());
	    dlclose(dl_handle);
            return 1;
        }
    }
