...
#define RGB_POS 3
float color_arr[width][height][RGB_POS];

/*function to be passed in create_thread*/
void* render (void* thread_num ){
    ...
    for( int px=*(int *)thread_num ; px<width; px += nthreads ){
	   ...
	   /*Put result into an array tp be printed later, make sure use one line per pixel*/	
	  color_arr[px][py][0] = scaled_color[0];
	  color_arr[px][py][1] = scaled_color[1];
	  color_arr[px][py][2] = scaled_color[2];
	}	
    }
   return NULL;
}
...
main( int argc, char **argv )
{
   ...
   int threads_num[nthreads];
   int i;
   for( i =0; i <nthreads; i++){
	threads_num[i] = i;
	int ret1 = pthread_create(&tids[i], NULL, render, &threads_num[i]);
	if(ret1){
		printf("Error creating threads. Error code is %d\n", ret1);
		exit(-1);
	}
    }
    /*Join threads, make sure all spawned threads are finished*/
    int j;
    for(j=0; j<nthreads; j++){
	int ret1 = pthread_join(tids[j], NULL);
	if(ret1){
		printf("Error joining thread. Error code is %d\n", ret1);
		exit(-1);
	}
    }
   /*Writing out results to disk*/
   int k,m;
   for(k=0; k<width; k++){
	for(m=0; m<height ; m++){
             ...
             */
            printf( "%.0f %.0f %.0f\n",
		    color_arr[k][m][0], color_arr[k][m][1], color_arr[k][m][2]);
        }
        printf( "\n" );
   }
